...
class EncryptedSocket extends EventEmitter {
    ...
    _authHost(hostKey) {
        ...
        function receiveAuthSuccess(data) {
            if (data.equals(SUCCESS)) { self.emit('connection') }
        }
    ...
    _authPeer(socket, publicKey, secretKey) {
        ...
        function receiveAuthRequest(data) {
            ...
            if (self.publicKey.equals(peerPublicKey)) { // return
    ...
    write(data) {
        if (!this.sharedKey) { throw new Error(`EncryptedSocket failed to write. Missing 'sharedKey'`) }
    ...
    _onReceive(data) {
        if (!this.sharedKey) { throw new Error(`EncryptedSocket failed to receive. Missing 'sharedKey'`) }
        ...
        if (!msg) { throw new Error('EncryptedSocket failed to decrypt received data.') }
        this.emit('data', msg)
    }
