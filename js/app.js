...
const swd	 = require('dat-swarm-defaults')
const mob	 = require('discovery-swarm')
const ram	 = require('random-access-memory')
const path   = require('path')
const events = require('events')
const sodium = require('sodium-universal')

const { EncryptedSocket, signalPeer } = require('./net');

const FRIENDSWARM  = new Buffer('swarm2')
const DEFAULT_PORT = 8080
const SWARM_OPTS   = { hash: false }
...
class App {
	...
	initLocalSwarm() {
		...
		const swarm = mob(swd(SWARM_OPTS))
	...
	updateUI() {
		...
		} else { this.$.friendsList.innerHTML = 'Nobody' }
	...
	async onSaveProfile() {
		...
		const profile = { name: form.name.value }
	...
	onSendChat() {
		if (!this.$.chatForm.checkValidity()) { return }
		const message = this.$.chatForm.message.value
		this.chat.sendMessage(message)
		this.$.chatForm.message.value = '';
	}

	onDisconnectChat() { if (this.chat) { this.chat.close() } }
...
class ChatRoom extends events {
	...
	get peerId() { return key2str(this.peerKey) }
	...
	dispatch(action) {
		switch (action.type) {
			case 'message':
				...
				break;
			default:
				console.warning(`Unknown msg type=${action.type}`, action)
		}
	}
	...
	static get(dirOrStorage, opts = {}) {
		return new Promise((resolve, reject) => {
			Dat(dirOrStorage, opts, (err, dat) => {
				...
				dat.joinNetwork(err => {
					reject(err)
					if (!dat.network.connected || !dat.network.connecting) { reject('Failed to load archive') }
				});
...
class FriendLoader extends events {
	constructor() { super()
		this.cache = new Map()
	}
